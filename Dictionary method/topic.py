...
# or the dict constructor. We will learn about the fromkey
...
# become the keys of a new dicitonary. The value parameter is
# optional and defaults to None, but the use can specify a value
...
# The word was added succesfully! But now we want to create a
# dictionary that would store the names of the staellites for those 
...
# assigned to all planets in our dicitonary. This happened because
# the fromkeys method assigns the same object to all keys.
# While referring to different keys of the planets_dict
# dictionary, we are stil referring to the same list. The difference
# from the previous example is that if we use mutable objects (a
# list, a dictionary) as values, all changes will also apply to our
...
# with different task depending on your needs.
...
# print(testable['June'])  # throws KeyError
...
# There are two ways that remove an tiem from the dictionary
...
# asisgns it to our variable. Let's go back to the example above
...
# - a new fromekys method for alternative dictionary
