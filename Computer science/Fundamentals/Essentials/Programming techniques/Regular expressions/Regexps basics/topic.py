...
# differences in syntax called "flavors". In this topic, we will
...
# - the word SPARK matches our pattern because it has a
#   suitable substring;
# - the word PARKING matches our pattern due to the same
#   reason;
# - the work MARK doesn't match out patter because of the
#   M letter;
# - the word QUARKS doesn't match out pattern since it does
#   not have a suitable part.

# To sum up, only three words match the PARK pattern.

#       In regular expressions, the case of characters is relevant:
#       park is not the same as PARK, i.e., they do not match.

# In addition, let's consider another sequence of characters PAKR.
# It does not match our pattern since two characters have a
# wrong (reverse) order.

# 3. The power of regular expressions
# Finding substrings is not very impressive, though. The real
# power of regular expressions comes when you start using
# special metacharacters called wildcards. They allow you to
# define a pattern, so you can match strings that do not
# necessarily contain an identical sequence of characters. You can
# skip some characters in a string or match different characters in
# the same positions, or even repeat a character several times.

# Let's introduce two simplest wildcards: dot and question mark.

# 4. The dot character
# The dot character . matches any single character including
# letters, digits, and so on, except for the newline character,
# unless it is specified.

# Let's look at our previous example again with several additional
# words.

# As you remember, in the previous example, two words did not
# match the pattern because of one unsuitable character. Let's
# consider them and also add two additional words. Here is our
# new patter .ARK with the dot character. It says: there is any
# character followed by ARK".
...
# But the WARM word does not match it. Think for a minute, how
# can this be fixed?

# The answer is to use the .AR. pattern, which is matched by
# WARM, CLARA, PART, and so on.

# The word ARK also does not match our pattern since it does not
# have a character in the . position in the pattern, while this is
# required.

# 5. The question mark
# The question mark ? is a special character that has "the
# preceding character or nothing" meaning. The question mark ?
# signals that the character before it can occur once or zero times
# in a string to match the pattern. When can we use it?

# Maybe with this example, you will finally begin to feel the magic
# of regexps. Consider the difference between English and
# American spelling. Imagine, we are trying to find all the studies
# mentioning color blindness in some publications archive.
# However, it contains different sources and their spelling may
# vary. What word should we look for? The answer is both!

# The pattern colou?r will match the strings colour and color,
# but not the string coloor. It is also possible to include the
# possibility of different letter cases to match the uppercase
# "Color" as well. We will learn how it is done in later topics.

# Let's return to our previous example. The word ARK does not
# match the .ARK pattern. But if we add ? right after the dot
# character .?ARK, the word ARK will match the new pattern
# since the first character is optional now.
