...
# independent structures, so getting familiar with just one of
# them means understanding a whole bunch of applications that
# share it. It is basically a general language to express ideas, and
# Model-View-Controller (MVC) is one particularly useful pattern
# to learn, since many popular frameworks like Django (Python),
# Spring (Java), and Ruby On Rails (Ruby) are using it.
...
#   access it, how to check it, and how data are related to
#   each other.
...
# corresponding components: Model, Templates and Views
# (MTV):
# - The Model->Model: the Model stays the same as in MVC.
# - The View->Templates: receiving and displaying data to
...
# standard names of MVC, feel free to read the FAQ section about
# it.
...
# switching links and pages, but what's happening under the
# hood? Let's see:
...
#    created.
# 3. The Views receives the requests.
...
# package. You should separate the work with each component as
# Django developers do: this way other developers will
# understand your code and you will understand theirs. We will
# cover it in more detail in the following topics.

# 3. Conclusion
# You've learned the Model-View-Controller pattern that is used
# in many popular web frameworks. It is built on the three
# components:
# 1. Model is in charge of the data;
# 2. View deals with receiving data and displaying it to the
#    user;
...
# We hope it doesn't confuse you too much! In our topics, we'll
# use the MTV terminology.
